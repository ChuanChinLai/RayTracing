...
void LaiEngine::CUDA::Init(int nx, int ny, int tx, int ty)
{

}

void LaiEngine::CUDA::Update(uint8_t* outputBuffer, const size_t buffer_size, int nx, int ny, int tx, int ty)
{
	clock_t start, stop;
	...
	start = clock();
	init_randState << <blocks, threads >> > (nx, ny, d_randState);
...
__global__ void LaiEngine::CUDA::create_world(LaiEngine::GameObject ** d_list, LaiEngine::GameObject ** d_world, LaiEngine::CUDA::Camera ** d_camera)
{
...
__global__ void LaiEngine::CUDA::free_world(LaiEngine::GameObject ** d_list, LaiEngine::GameObject ** d_world, LaiEngine::CUDA::Camera ** d_camera)
{
	delete *(d_list);
	delete *(d_list + 1);
	delete *d_world;
...
__global__ void LaiEngine::CUDA::init_randState(int max_x, int max_y, curandState * randState)
{
...
__device__ glm::vec3 LaiEngine::CUDA::get_colors(const LaiEngine::Ray & r, LaiEngine::GameObject ** world)
{
...
__device__ bool LaiEngine::CUDA::hit_sphere(const glm::vec3 & center, float radius, const LaiEngine::Ray & ray)
{
